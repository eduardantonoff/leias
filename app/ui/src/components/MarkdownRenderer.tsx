...
const SAFE_IFRAME_PATHS = new Set([
  "/media/iframes/quizzes/multiple_choice.html",
  "/media/iframes/quizzes/true_false.html",
  "/media/iframes/graph/retrieval_graph.html",
  "/media/iframes/interactive/linear_regression.html",
  "/media/iframes/interactive/clustering.html",
]);
...
const normalizeKgIframeSrc = (src: string) => {
  if (!src.startsWith("/kg/")) return src;
  const candidateId = decodeURIComponent(src.slice(4).split("?")[0].split("#")[0] || "");
  if (!candidateId) return src;

  const params = new URLSearchParams();
  params.set("id", candidateId);
  return `${window.location.origin}/media/iframes/graph/retrieval_graph.html?${params.toString()}`;
};
...
const normalizeIframeSrc = (src: string) => {
  if (!src) return "";
  const normalizedSrc = src.startsWith("/kg/") ? normalizeKgIframeSrc(src) : src;
...
const iframeHeaderForSrc = (src: string) => {
  if (src.includes("multiple_choice.html")) return "choose one";
  if (src.includes("true_false.html")) return "true or false";
  if (src.includes("retrieval_graph.html")) return "knowledge map";
  if (src.includes("linear_regression.html")) return "linear regression";
  if (src.includes("clustering.html")) return "clustering";
  return "sandbox";
};

const isDynamicIframeSrc = (src: string) =>
  src.includes("linear_regression.html") || src.includes("clustering.html");

const iframeSizingForSrc = (src: string) => {
  if (src.includes("multiple_choice.html") || src.includes("true_false.html")) {
    return {
      paddingBottom: "56.25%",
      minHeight: "clamp(300px, 82vw, 360px)",
    };
  }
  if (src.includes("linear_regression.html")) {
    return {
      paddingBottom: "78%",
      minHeight: "620px",
    };
  }
  if (src.includes("clustering.html")) {
    return {
      paddingBottom: "80%",
      minHeight: "520px",
    };
...
const IframeEmbed = memo(({ src }: { src: string }) => {
  const headerText = iframeHeaderForSrc(src);
  const sizing = iframeSizingForSrc(src);
  const isDynamic = isDynamicIframeSrc(src);
  const isKnowledgeGraph = src.includes("retrieval_graph.html");
  const isQuiz = src.includes("multiple_choice.html") || src.includes("true_false.html") || src.includes("quiz.html");
  const frameless = isKnowledgeGraph || isQuiz;
  const iframeRef = useRef<HTMLIFrameElement | null>(null);
  const [dynamicHeight, setDynamicHeight] = useState<number>(() => {
    const parsed = Number.parseInt(sizing.minHeight, 10);
  ...
  useEffect(() => {
    ...
    const updateHeightFromDocument = () => {
      try {
        ...
        if (measured > 0) {
          setDynamicHeight((prev) => (prev === measured ? prev : measured));
        }
      } catch {
        return;
      }
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.type !== "iframe-height") return;
      const incoming = Number(data.height);
      if (!Number.isFinite(incoming)) return;
      const parsedMin = Number.parseInt(sizing.minHeight, 10);
      const minHeight = Number.isFinite(parsedMin) ? parsedMin : 280;
      const clamped = Math.max(minHeight, Math.min(1800, Math.round(incoming)));
      setDynamicHeight((prev) => (prev === clamped ? prev : clamped));
    };

    const onLoad = () => {
      updateHeightFromDocument();
      window.setTimeout(updateHeightFromDocument, 120);
      window.setTimeout(updateHeightFromDocument, 420);
    };
  ...
  }, [isDynamic, sizing.minHeight]);

  return (
    <div className={frameless ? "my-6 overflow-hidden" : "my-6 rounded-2xl overflow-hidden bg-card/95 border border-[#b5b5b5]"}>
      {!frameless && (
        <div className="px-4 py-1.5 bg-muted/55 flex items-center">
          <span className="text-[10px] tracking-wide uppercase font-medium text-ink-soft">{headerText}</span>
        </div>
      )}
