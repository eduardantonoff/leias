...
class Resource(BaseModel):
    type: ResourceType = Field(description="Resource type to embed in the plan.")
    url: str = Field(description="Resource link from the knowledge graph.")
    caption: str = Field(
        min_length=16,
        max_length=256,
...
class Checkpoint(BaseModel):
    type: CheckpointType = Field(description="Verification format to use for this checkpoint.")
...
class Plan(BaseModel):
    topic: str = Field(description="Topic name exactly as it appears in the knowledge graph.")
    topic_id: str = Field(description="Knowledge graph topic ID for this plan.")
    title: str = Field(
        min_length=16,
        max_length=128,
