...
const ANSWER_SUBMITTED_PLACEHOLDER = '--- answer submitted ---';
...
const getPlanStatusLabel = (planContext: PlanContext) => {
  if (planContext.completion_message) return planContext.completion_message;
  if (planContext.update_message) return planContext.update_message;
  return planContext.creation_message ?? 'Learning plan created';
};
    ...
    return (
      <div className="w-full mt-2">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {planContext && (
            <div className={logChipClass}>
              <span className="text-[10px]">{getPlanStatusLabel(planContext)}</span>
            </div>
          )}
        </div>
      </div>
...
export const PreviewMessage: FC<PreviewMessageProps> = memo(
  ({ message, logs = [], source, isStreaming = false, isToolCallActive = false, isNew = false, activeSource }) => {
    ...
    const isAnswerSubmittedMarker =
      isUserMessage && message.content.trim().toLowerCase() === ANSWER_SUBMITTED_PLACEHOLDER;
    const shouldEnableCollapse = useMemo(() => {
      if (!isUserMessage || !message.content || isAnswerSubmittedMarker) return false;
      const text = message.content.trim();
      const newlineCount = (text.match(/\n/g) || []).length;
      return text.length > 120 || newlineCount >= 3;
    }, [isAnswerSubmittedMarker, isUserMessage, message.content]);
    ...
    return (
      ...
      >
        <div
          className={cn(
            messageWrapperClass,
            isUserMessage && !isAnswerSubmittedMarker && 'justify-end'
          )}
        >
          {isAnswerSubmittedMarker ? (
